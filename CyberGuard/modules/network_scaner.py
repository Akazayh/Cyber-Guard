import re
from utils import run_command_safe, validate_ip, validate_ip_network, validate_filename, ensure_directories
import os
import sys
import time
...
def get_ip(prompt=Fore.BLUE + "Enter Target IP:"):
    while True:
        ...
        if not value:
            ...
            continue
        if '/' in value:
            if validate_ip_network(value):
                return value
        else:
            if validate_ip(value):
                return value
        print(Fore.RED + "Invalid IP or network")
def get_ports():
    while True:
        value = input(Fore.BLUE + "Enter ports (ex: 80,443 or 80-100): ").strip()
        if not value:
            print(Fore.RED + "Ports cannot be empty")
            continue
        if re.match(r'^[\d,-]+$', value):
            return value
        print(Fore.RED + "Invalid format. Use digits, commas, or dash only.")
def get_fake_ip(prompt=Fore.BLUE + "Enter Fake IP:"):
    while True:
        value = input(prompt).strip()
        if value == "":
            print(Fore.RED + Style.BRIGHT + "IP cannot be empty!!!")
            continue
        if '/' in value:
            if validate_ip_network(value):
                return value
            else:
                print(Fore.RED + Style.BRIGHT + "Invalid network address (e.g., 192.168.1.0/24)")
        else:
            if validate_ip(value):
                return value
            else:
...
def ensure_archives_dir():
	...
	if not os.path.exists("Archives_Nmap"):
		...
		logging.info("Created Archives_Nmap directory")
def run_nmap(cmd_list, output_file=None):
    if output_file:
        cmd_list.extend(['-oN', output_file])
    print(Fore.CYAN + f"Running: {' '.join(cmd_list)}")
    logging.info("Running nmap scan: " + ' '.join(cmd_list))
    ret, stdout, stderr = run_command_safe(cmd_list, timeout=600)
    if ret != 0:
        print(Fore.RED + f"nmap error: {stderr}")
        logging.error("nmap error: " + stderr)
    else:
        print(Fore.GREEN + "Scan completed")
        if stdout:
            print(Fore.WHITE + stdout)
        if stderr:
            print(Fore.YELLOW + stderr)
...
while True:
	...
	if choix == "1":
		...
		if rep == "Y":
			...
			if target_ip:
				run_nmap(["nmap", target_ip], filepath)
		...
		if rep == "Y":
			...
			filepath = os.path.join("Archives_Nmap", fil + ".txt")
			if target_ip:
				run_nmap(["nmap", "-sn", target_ip], filepath)
				print(Fore.GREEN + f"✓ Results saved to: {filepath}")
				input("\nPress Enter to go back to the menu")
		else:
			if target_ip:
		...
		if rep == "Y":
			...
			if port and target_ip:
				run_nmap(["nmap", "-p", port, target_ip], filepath)
				print(Fore.GREEN + f"✓ Results saved to: {filepath}")
				input("\nPress Enter to go back to the menu")
		else:
			if port and target_ip:
				run_nmap(["nmap", "-p", port, target_ip])
				input("\nPress Enter to go back to the menu")
		...
		if reponse == "Y":
			...
			if rep == "Y":
				...
				filepath = os.path.join("Archives_Nmap", fil + ".txt")
				if target_ip:
					run_nmap(["nmap", "-p-", target_ip], filepath)
					print(Fore.GREEN + f"✓ Results saved to: {filepath}")
					input("\nPress Enter to go back to the menu")
			else:
				if target_ip:
					run_nmap(["nmap", "-p-", target_ip])
					input("\nPress Enter to go back to the menu")
		else:
		...
		if rep == "Y":
			...
			filepath = os.path.join("Archives_Nmap", fil + ".txt")
			if target_ip:
				run_nmap(["nmap", "-p", ports, target_ip], filepath)
				print(Fore.GREEN + f"✓ Results saved to: {filepath}")
				input("\nPress Enter to go back to the menu")
		else:
			if target_ip:
				run_nmap(["nmap", "-p", ports, target_ip])
				input("\nPress Enter to go back to the menu")
		...
		if rep == "Y":
			...
			filepath = os.path.join("Archives_Nmap", fil + ".txt")
			if target_ip:
				run_nmap(["nmap", "-sV", target_ip], filepath)
				print(Fore.GREEN + f"✓ Results saved to: {filepath}")
				input("\nPress Enter to go back to the menu")
		else:
			if target_ip:
		...
		if rep == "Y":
			...
			filepath = os.path.join("Archives_Nmap", fil + ".txt")
			if target_ip:
				run_nmap(["nmap", "-O", target_ip], filepath)
				print(Fore.GREEN + f"✓ Results saved to: {filepath}")
				input("\nPress Enter to go back to the menu")
		else:
			if target_ip:
				run_nmap(["nmap", "-O", target_ip])
				input("\nPress Enter to go back to the menu")
		...
		if reponse == "Y":
			...
			if rep == "Y":
				...
				filepath = os.path.join("Archives_Nmap", fil + ".txt")
				if target_ip:
					run_nmap(["nmap", "-A", target_ip], filepath)
					print(Fore.GREEN + f"✓ Results saved to: {filepath}")
					input("\nPress Enter to go back to the menu")
			else:
				if target_ip:
					run_nmap(["nmap", "-A", target_ip])
					input("\nPress Enter to go back to the menu")
		else:
			print("Cancelled by user")
			time.sleep(1.5)
		...
		while True:
			...
			if script == "1":
				...
				if rep == "Y":
					...
					filepath = os.path.join("Archives_Nmap", fil + ".txt")
					if target_ip:
						run_nmap(["nmap", "--script", "vuln", target_ip], filepath)
						print(Fore.GREEN + f"✓ Results saved to: {filepath}")
						input("\nPress Enter to go back to the menu")
				else:
					if target_ip:
				...
				if rep == "Y":
					...
					filepath = os.path.join("Archives_Nmap", fil + ".txt")
					if target_ip:
						run_nmap(["nmap", "--script", "http-vuln-cve2006-3392", target_ip], filepath)
						print(Fore.GREEN + f"✓ Results saved to: {filepath}")
						input("\nPress Enter to go back to the menu")
				else:
					if target_ip:
						run_nmap(["nmap", "--script", "http-vuln-cve2006-3392", target_ip])
						input("\nPress Enter to go back to the menu")
			elif script == "3":
				target_ip = get_ip()
				rep = input("Do you want to save the results? (y=Yes/n=No):").strip().upper()
				if rep == "Y":
					...
					filepath = os.path.join("Archives_Nmap", fil + ".txt")
					if target_ip:
						run_nmap(["nmap", "--script", "smb-vuln-ms17-010", target_ip], filepath)
						print(Fore.GREEN + f"✓ Results saved to: {filepath}")
						input("\nPress Enter to go back to the menu")
				else:
					if target_ip:
						run_nmap(["nmap", "--script", "smb-vuln-ms17-010", target_ip])
						input("\nPress Enter to go back to the menu")
			elif script == "4":
				target_ip = get_ip()
				rep = input("Do you want to save the results? (y=Yes/n=No):").strip().upper()
				if rep == "Y":
					...
					filepath = os.path.join("Archives_Nmap", fil + ".txt")
					if target_ip:
						run_nmap(["nmap", "--script", "ftp-anon", target_ip], filepath)
						print(Fore.GREEN + f"✓ Results saved to: {filepath}")
						input("\nPress Enter to go back to the menu")
				else:
					if target_ip:
						run_nmap(["nmap", "--script", "ftp-anon", target_ip])
						input("\nPress Enter to go back to the menu")
			elif script == "5":
				target_ip = get_ip()
				rep = input("Do you want to save the results? (y=Yes/n=No):").strip().upper()
				if rep == "Y":
					...
					filepath = os.path.join("Archives_Nmap", fil + ".txt")
					if target_ip:
						run_nmap(["nmap", "--script", "ssh-brute", target_ip], filepath)
						print(Fore.GREEN + f"✓ Results saved to: {filepath}")
						input("\nPress Enter to go back to the menu")
				else:
					if target_ip:
						run_nmap(["nmap", "--script", "ssh-brute", target_ip])
						input("\nPress Enter to go back to the menu")
			elif script == "0":
				break       
			else:
		...
		if rep == "Y":
			...
			filepath = os.path.join("Archives_Nmap", fil + ".txt")
			if target_ip:
				run_nmap(["nmap", "-sS", target_ip, filepath], filepath)
				print(Fore.GREEN + f"✓ Results saved to: {filepath}")
				input("\nPress Enter to go back to the menu")
		else:
			if target_ip:
				run_nmap(["nmap", "-sS", target_ip])
				input("\nPress Enter to go back to the menu")
		...
		if rep == "Y":
			...
			filepath = os.path.join("Archives_Nmap", fil + ".txt")
			if target_ip:
				run_nmap(["nmap", "-T4", "-A", "-v", target_ip], filepath)
				print(Fore.GREEN + f"✓ Results saved to: {filepath}")
				input("\nPress Enter to go back to the menu")
		else:
			if target_ip:
				run_nmap(["nmap", "-T4", "-A", "-v", target_ip])
				input("\nPress Enter to go back to the menu")
		...
		if rep == "Y":
			...
			filepath = os.path.join("Archives_Nmap", fil + ".txt")
			if target_ip:
				run_nmap(["nmap", "-f", target_ip], filepath)
				print(Fore.GREEN + f"✓ Results saved to: {filepath}")
				input("\nPress Enter to go back to the menu")
		else:
			if target_ip:
				run_nmap(["nmap", "-f", target_ip])
				input("\nPress Enter to go back to the menu")
		...
		if rep == "Y":
			...
			filepath = os.path.join("Archives_Nmap", fil + ".txt")
			if target_ip:
				run_nmap(["nmap", "-Pn", target_ip], filepath)
				print(Fore.GREEN + f"✓ Results saved to: {filepath}")
				input("\nPress Enter to go back to the menu")
		else:
			if target_ip:
				run_nmap(["nmap", "-Pn", target_ip])
				input("\nPress Enter to go back to the menu")
		...
		if reponse == "Y":
			...
			if rep == "Y":
				...
				filepath = os.path.join("Archives_Nmap", fil + ".txt")
				if target_ip:
					run_nmap(["nmap", "-sU", target_ip], filepath)
					print(Fore.GREEN + f"✓ Results saved to: {filepath}")
					input("\nPress Enter to go back to the menu")
			else:
				if target_ip:
					run_nmap(["nmap", "-sU", target_ip])
					input("\nPress Enter to go back to the menu")
		else:
			print("Cancelled by user")
			time.sleep(1.5)
		...
		if rep == "Y":
			...
			filepath = os.path.join("Archives_Nmap", fil + ".txt")
			if target_ip:
				run_nmap(["nmap", "-6", target_ip], filepath)
				print(Fore.GREEN + f"✓ Results saved to: {filepath}")
				input("\nPress Enter to go back to the menu")
		else:
			if target_ip:
				run_nmap(["nmap", "-6", target_ip])
				input("\nPress Enter to go back to the menu")
		...
		if rep == "Y":
			...
			filepath = os.path.join("Archives_Nmap", fil + ".txt")
			if target_ip:
				run_nmap(["nmap", "-S", fake_ip, "-e", "eth0", "-Pn", target_ip], filepath)
				print(Fore.GREEN + f"✓ Results saved to: {filepath}")
				input("\nPress Enter to go back to the menu")
		else:
			if target_ip:
				run_nmap(["nmap", "-S", fake_ip, target_ip])
				input("\nPress Enter to go back to the menu")
		...
		if rep == "Y":
			...
			filepath = os.path.join("Archives_Nmap", fil + ".txt")
			if target_ip:
				run_nmap(["nmap", target_ip, "--spoof-mac", "0"], filepath)
				print(Fore.GREEN + f"✓ Results saved to: {filepath}")
				input("\nPress Enter to go back to the menu")
		else:
			if target_ip:
				run_nmap(["nmap", target_ip, "--spoof-mac", "0"])
				input("\nPress Enter to go back to the menu")
		...
		if rep == "Y":
			...
			filepath = os.path.join("Archives_Nmap", fil + ".txt")
			if target_ip:
				run_nmap(["nmap", "-D", "RND:10", target_ip], filepath)
				print(Fore.GREEN + f"✓ Results saved to: {filepath}")
				input("\nPress Enter to go back to the menu")
		else:
			if target_ip:
				run_nmap(["nmap", "-D", "RND:10", target_ip])
				input("\nPress Enter to go back to the menu")
	elif choix=="99":
		os.system("clear")
		print(Fore.WHITE + """
