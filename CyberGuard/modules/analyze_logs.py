...
def analyze_file(path):
	...
	try:
		with open(path, 'r', encoding='utf-8', errors='ignore') as f:
			logging.info("Analyzing log file: " + path)			
		...
		logging.error("Error reading file " + path + ": " + str(e))
		return None
	return summary
...
def search_in_file(path, keyword):
	...
	try:
		with open(path, 'r', encoding='utf-8', errors='ignore') as f:
			logging.info("Searching for keyword: " + keyword + " in file: " + path)
			for lineno, raw in enumerate(f, start=1):
				line = raw.rstrip('\n')
		...
		logging.error("Error searching file " + path + ": " + str(e))
		return []
	return matches
...
def detect_anomalies(path):
	...
	try:
		with open(path, 'r', encoding='utf-8', errors='ignore') as f:
			logging.info("Detecting anomalies in file: " + path)
			for lineno, raw in enumerate(f, start=1):
				line = raw.rstrip('\n')
				if not line:
					continue
...
def choose_file_from_dir(directory):
	...
	return target
def export_json(summary, anomalies, outpath):
    data = {
        "file": summary['file'],
        "total_lines": summary['total_lines'],
        "session_entries": summary['session_entries'],
        "protocols": dict(summary['protocols']),
        "attack_types": dict(summary['attack_types']),
        "top_ips": dict(summary['ips'].most_common(20)),
        "anomalies": {
            "attack_counts": dict(anomalies['attack_counts']),
            "suspicious_ips": dict(anomalies['suspicious_ips'].most_common(20))
        }
    }
...
def cli_mode():
    parser = argparse.ArgumentParser(description="IDS Log Analyzer")
    parser.add_argument("file", help="Log file to analyze")
    parser.add_argument("--json", help="Export to JSON file")
    parser.add_argument("--txt", help="Export to text report")
    ...
    if not summary:
        sys.exit(1)
    anomalies = detect_anomalies(args.file)
    if args.json:
        if export_json(summary, anomalies, args.json):
            print(Fore.GREEN + f"JSON saved to {args.json}")
    if args.txt:
        if export_report(summary, anomalies, args.txt):
            print(Fore.GREEN + f"Report saved to {args.txt}")
